        ...
        <div class="col-12">
          ...
          <div class="box">
            ...
            <form  method="post">
              ...
              <button type="submit" class="btnupdate  btn-lg btn-danger" style="height: 40px; width:200px; font-size: 15px">Update</button>
            </form> 
          </div>
          </div>
      <!-- ./row -->
...
<script>
  $(".myadmin-alert .closed").click(function(event) {
   ...
   <script>
    // var element = CKEDITOR.document.getygetById('myElement');
  ...
  <script>
    ...
    <script>
      ...
      $.ajax({
        ...
        CKEDITOR.instances.ckeditor.insertHtml(response[index].context);
          // alert(response[index].context);
         });
    ...
    headers: {
      ...
      var data = CKEDITOR.instances.ckeditor.getData();
        $.ajax({
         type:"post",
        url:"{{url('/update')}}",
        data:{'data':data,'scheme_id':id},
        success:function(response){
           successMessage();
           window.open('/schemedetails/'+id,'_blank');
        },
        error:function(error){
         console.log(error);
        }
